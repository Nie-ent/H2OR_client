...
import { Users, Home, LayoutDashboard } from 'lucide-react';
  ...
  return (
    <aside className="w-64 bg-[#0b2545] text-white hidden md:flex flex-col h-screen sticky top-0">
      ...
      <nav className="flex-1 p-4 space-y-2">
        ...
        <Link 
          to="/admin" 
          className={`flex items-center gap-3 p-3 rounded-lg transition-colors cursor-pointer ${isActive('/admin')}`}
        >
        ...
        <Link 
          to="/admin/users" 
          className={`flex items-center gap-3 p-3 rounded-lg transition-colors cursor-pointer ${isActive('/admin/users')}`}
        >
          <Users size={20} /> 
